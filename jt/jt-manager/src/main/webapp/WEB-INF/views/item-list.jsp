...
<script>
    ...
    function getSelectionsIds(){
    	...
    	//将数组拼接成串 1,2,3,4,5   ，springmvc中参数以字符串的形式传递
    	ids = ids.join(",");
    	return ids;
    }
    ...
    var toolbar = [{
        	...
        	$("#itemEditWindow").window({
        		onLoad :function(){
                    ...
                    $.get("item/cat/queryItemName",{"itemCatId":cid},function (name) {
                        $("#selected").prev("span").text(name);//html传递的时标签元素，效果一样，但严格意义text更合适
        ...
        handler:function(){
        	...
        	$.messager.confirm('确认','确定下架ID为 '+ids+' 的商品吗？',function(r){
        	    if (r){
        	    	var params = {"ids":ids};
                	$.get("/item/2",params, function(data){
            			if(data.status == 200){
        ...
        handler:function(){
        	...
        	$.messager.confirm('确认','确定上架ID为 '+ids+' 的商品吗？',function(r){
        	    if (r){
        	    	var params = {"ids":ids};
                	$.get("/item/1",params, function(data){
            			if(data.status == 200){
